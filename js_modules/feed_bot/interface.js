...
async function notify() {
    ...
    if (accounts && accounts.length > 0) {
        for (let acc of accounts) {
                try {
                    let posts = await methods.getFeed(acc.login);
    let last_post = posts[0].entry_id;
let feed = [];
    feed[0] = `${lng[acc.lng].feed_name} <a href="https://dpos.space/serey/profiles/${acc.login}">${acc.login}</a>`;
let posts_counter = 0;
let msg_counter = 0;
for (let post of posts) {
    if (post.id === acc.last_post) break;
posts_counter++;
let reblog_by_str = '';
...
let chunk = `
${posts_counter}. <a href="https://serey.io/authors/${post.author}/${post.permlink}">${post.title}</a> ${lng[acc.lng].from} <a href="https://dpos.space/serey/profiles/${post.author}">@${post.author}</a>${reblog_by_str}`;
...
async function commentOperation(content, op, opbody, timestamp) {
    ...
    let tags_list = '';
for (let tag of tags) {
    tags_list += ` <a href="https://serey.io/categories/${tag}/new">#${tag}</a>`;
}
